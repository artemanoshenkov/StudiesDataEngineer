...
def reading_processing_data() -> None:
    ...
    try:
        ...
        df = df.withColumn(
            "maintenance_year", df["maintenance_year"].cast(IntegerType())
        )
...
def query_execution() -> None:
    ...
    try:
        ...
        avg_year.write.jdbc(
            url=url, table="avg_year", mode="append", properties=properties
        )
        ...
        median_year.write.jdbc(
            url=url, table="median_year", mode="append", properties=properties
        )

        # топ-10 областей и городов с наибольшим количеством объектов
        top_regions = (
            df.groupBy("region")
            .agg(F.count("description").alias("count_object"))
            .orderBy(F.col("count_object").desc())
            .limit(10)
        )

        creating_data_table("top_regions", top_regions.dtypes)

        top_regions.write.jdbc(
            url=url, table="top_regions", mode="append", properties=properties
        )
        ...
        top_cities.write.jdbc(
            url=url, table="top_cities", mode="append", properties=properties
        )

        # Здания с максимальной и минимальной площадью в рамках каждой области
        buildings_max_min = df.groupBy("region").agg(
            F.max("square").alias("max_square"), F.min("square").alias("min_square")
        )

        creating_data_table("buildings_max_min", buildings_max_min.dtypes)

        buildings_max_min.write.jdbc(
            url=url, table="buildings_max_min", mode="append", properties=properties
        )

        # Количество зданий по десятилетиям
        buildings_with_decade = df.withColumn(
            "decade", (F.col("maintenance_year") / 10).cast("int") * 10
        )
        ...
        building_in_yer.write.jdbc(
            url=url, table="building_in_yer", mode="append", properties=properties
        )
    except Exception as error:
        print(f"Возникла ошибка: {error}")
    else:
        print(
            "Запросы успешно выполнены, результаты записаны в db clickhouse по таблицам запросов"
        )


# Настройка сессии Spark
spark = (
    SparkSession.builder.appName("MyAppRuHouses")
    .master("local[*]")
...
with DAG(
    ...
    task_2 = PythonOperator(
        task_id="query_execution", python_callable=query_execution, dag=dag
    )
